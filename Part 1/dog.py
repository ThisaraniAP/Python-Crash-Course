# 16/06/2021
class Dog:
	"""A simple attempt to model a dog."""

	def __init__(self, name, age):
		"""Initialize name and age attributes."""
		self.name = name
		self.age = age

	def sit(self):
		"""Simulate a dog sitting in response to a command."""
		print(f"{self.name} is now sitting.")

	def roll_over(self):
		"""Simulate rolling over in response to a command."""
		print(f"{self.name} rolling over!")

# a capitalized name normally refers to a class
# a function that is part of a class is a called method
# the __init__() is a method has 2 leading and trailing underscores


my_dog = Dog('Willie', 6)

print(f"My dog's name is {my_dog.name}.")
print(f"My dog's age is {my_dog.age} years old.")
my_dog.sit()
my_dog.roll_over()

# you can create multiple instances
my_dog = Dog('Willie', 6)
your_dog = Dog('Lucy', 3)

print(f"\nMy dog's name is {my_dog.name}.")
print(f"My dog's age is {my_dog.age} years old.")
my_dog.sit()

print(f"\nMy dog's name is {your_dog.name}.")
print(f"My dog's age is {your_dog.age} years old.")
your_dog.sit()

