...
class Admin(User):
	...
	def __init__(self, first_name, last_name, username, location, age, login_attempts, Privileges):
		
		"""Initialize the admin's attributes."""
		super().__init__(first_name, last_name, username, location, age, login_attempts)
		self.privileges = Privileges

mori = Privileges(['can add post', 'can delete post', 'can ban user'])
mori_admin = Admin('Mori', 'Jin', 'Monkey King', 'Korea', 17, 5, mori.privileges)
